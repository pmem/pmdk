...
class BinaryOutputHandler:
    ...
    def add_file(self, file, map_base, size):
        ...
        :return: None
        """
        binfile = BinaryFile(file, map_base, size, self._checker, self._logger)
        if binfile.not_in(self._files):
            self._files.append(binfile)
...
class BinaryFile(utils.Rangeable):
    ...
    def not_in(self, files):
        for f in files:
            if self._map_max - self._map_base == f._map_max - f._map_base \
                    and self._map_base == f._map_base:
                return False
        return True
