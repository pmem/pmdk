    ...
    def do_store(self, store_op):
        ...
        for i, bf in enumerate(self._files):
            if utils.range_cmp(store_op, bf) == 0:
                self._logger.debug(f"Doing store in file no. {i}: {bf}")
                bf.do_store(store_op)
                store_ok = True
        if not store_ok:
            raise OSError(f"No suitable file found for store {store_op}")
    ...
    def check_consistency(self):
        ...
        for bf in self._files:
            if not bf.check_consistency():
                raise InconsistentFileException(f"File {bf} inconsistent")
...
class BinaryFile(utils.Rangeable):
    ...
    def __init__(self, file_name, map_base, size, checker, logger):
        """
    ...
    def __str__(self):
        return (
            f"{self._file_name} (base: {hex(self._map_base)}, "
            f"size: {hex(self._map_max-self._map_base)})"
        )
    ...
    def do_store(self, store_op):
        ...
        self._logger.debug(
            "do_store: old_value: "
            f"{hex(int.from_bytes(store_op.old_value, byteorder=byteorder))}, "
            "new_value: "
            f"{hex(int.from_bytes(store_op.new_value, byteorder=byteorder))}"
        )
