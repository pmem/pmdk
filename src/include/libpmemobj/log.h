...
/*
 * Available log levels. Log levels (except for PMEMOBJ_LOG_DISABLED) are used in the logging API
 ...
 * - PMEMOBJ_LOG_THRESHOLD - the main threshold used to filter out undesired logging messages.
 *   Messages on a higher level than the primary threshold level are ignored. PMEMOBJ_LOG_DISABLED
 *   shall be used to suppress logging. The default value is PMEMOBJ_LOG_WARNING.
 * - PMEMOBJ_LOG_THRESHOLD_AUX - the auxiliary threshold intended for use inside the logging function
 *   (please see log_get_threshold(3)). The logging function may or may not take this
 *   threshold into consideration. The default value is PMEMOBJ_LOG_DISABLED.
 *
 * Available threshold levels are defined by enum log_level:
 * - PMEMOBJ_LOG_DISABLED - all messages will be suppressed
 * - PMEMOBJ_LOG_LEVEL_FATAL - an error that causes the library to stop working immediately
 * - PMEMOBJ_LOG_LEVEL_ERROR - an error that causes the library to stop working properly
 * - PMEMOBJ_LOG_LEVEL_WARNING - an error that could be handled in the upper level
 * - PMEMOBJ_LOG_LEVEL_NOTICE - non-massive info mainly related to public API function completions
 ...
 * the primary destination (PMEMOBJ_LOG_THRESHOLD applies) whereas stderr(3) is the secondary
 * destination (PMEMOBJ_LOG_THRESHOLD_AUX applies).
 *
 * RETURN VALUE
 * pmemobj_log_set_threshold() function returns 0 on success or a negative error code on
 * failure.
 *
 * ERRORS
 * pmemobj_log_set_threshold() can fail with the following errors:
 * - E_INVAL - threshold is not PMEMOBJ_LOG_THRESHOLD nor PMEMOBJ_LOG_THRESHOLD_AUX
 * - E_INVAL - level is not a value defined by enum log_level type
 * - E_AGAIN - a temporary error occurred, the retry may fix the problem
 *
 * SEE ALSO
 ...
 *
 * RETURN VALUE
 * pmemobj_log_get_threshold() function returns 0 on success or a negative error code on failure.
 *
 * ERRORS
 * pmemobj_log_get_threshold() can fail with the following errors:
 * - E_INVAL - threshold is not CORE_LOG_THRESHOLD nor CORE_LOG_THRESHOLD_AUX
 ...
 *
 * DESCRIPTION
 * pmemobj_log_set_function() allows choosing the function which will get all the generated logging
 * messages. The log_function can be either CORE_LOG_USE_DEFAULT_FUNCTION which will use
 ...
 *
 * THE DEFAULT LOGGING FUNCTION
 * The initial value of the logging function is CORE_LOG_USE_DEFAULT_FUNCTION. This function writes
 * messages to syslog(3) (the primary destination) and to stderr(3) (the secondary destination).
 *
 * RETURN VALUE
 * pmemobj_log_set_function() function returns 0 on success or error code on failure.
 *
 * ERRORS
 * - E_AGAIN - a temporary error occurred, the retry may fix the problem
 *
 * NOTE
 * The logging messages on the levels above the CORE_LOG_THRESHOLD level won't trigger the logging
 * function.
 *
