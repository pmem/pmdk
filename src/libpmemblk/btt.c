...
btt_write(struct btt *bttp, unsigned lane, uint64_t lba, const void *buf)
{
	...
	if (bttp->major == 1) {
		...
		put_aba_in_a_lane(arenap, lane,
			(old_entry&BTT_MAP_ENTRY_LBA_MASK));
	}
