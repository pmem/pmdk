...
struct tx_data {
	...
	jmp_buf env;
	enum pobj_tx_failure_event failure_event;
};
...
pmemobj_tx_begin(PMEMobjpool *pop, jmp_buf env, ...)
{
	...
	enum pobj_tx_failure_event failure_event = POBJ_TX_FAILURE_ABORT;
	...
	if (tx->stage == TX_STAGE_WORK) {
		...
		failure_event = txd->failure_event;
	...
	txd->failure_event = failure_event;
...
tx_abort_on_failure_flag(struct tx *tx)
{
	...
	if (txd->failure_event == POBJ_TX_FAILURE_RETURN)
...
/*
 * pmemobj_tx_set_failure_event -- enables or disables automatic transaction
 * abort in case of an error
 */
void
pmemobj_tx_set_failure_event(enum pobj_tx_failure_event event)
{
	LOG(3, "event %d", event);
	...
	txd->failure_event = event;
}

/*
 * pmemobj_tx_get_failure_event -- returns enum specifying failure event
 * for the current transaction.
 */
enum pobj_tx_failure_event
pmemobj_tx_get_failure_event(void)
{
	LOG(3, NULL);
