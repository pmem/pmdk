...
enum pobj_arena_assignment_type Default_arena_assignment_type =
	POBJ_ARENA_ASSIGNMENT_THREAD_KEY;

struct arena_thread_assignment {
	enum pobj_arena_assignment_type type;
	union {
		os_tls_key_t thread;
		struct arena *global;
	};
...
heap_arena_thread_attach(struct palloc_heap *heap, struct arena *a)
{
	...
	ASSERTeq(assignment->type, POBJ_ARENA_ASSIGNMENT_THREAD_KEY);
	...
	enum pobj_arena_assignment_type type)
{
	...
	switch (type) {
		case POBJ_ARENA_ASSIGNMENT_THREAD_KEY:
			ret = os_tls_key_create(&assignment->thread,
				heap_thread_arena_destructor);
			break;
		case POBJ_ARENA_ASSIGNMENT_GLOBAL:
		...
		struct stats *stats, struct pool_set *set)
{
	...
	if ((err = arena_thread_assignment_init(&h->arenas.assignment,
		Default_arena_assignment_type)) != 0) {
