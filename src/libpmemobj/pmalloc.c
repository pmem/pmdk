...
/*
 * CTL_WRITE_HANDLER(arena_assignment_type) -- sets the current arena assignment
 * type
 */
static int
CTL_WRITE_HANDLER(arena_assignment_type)(void *ctx,
	enum ctl_query_source source, void *arg, struct ctl_indexes *indexes)
{
	enum pobj_arena_assignment_type *src = arg;

	Default_arena_assignment_type = *src;

	return 0;
}

/*
 * CTL_READ_HANDLER(arena_assignment_type) -- reads the current arena assignment
 * type
 */
static int
CTL_READ_HANDLER(arena_assignment_type)(void *ctx,
	enum ctl_query_source source, void *arg, struct ctl_indexes *indexes)
{
	enum pobj_arena_assignment_type *dest = arg;

	*dest = Default_arena_assignment_type;

	return 0;
}

/*
 * arena_assignment_type_parser -- parses the arena assignment enum
 */
static int
arena_assignment_type_parser(const void *arg, void *dest, size_t dest_size)
{
	const char *vstr = arg;
	enum pobj_arena_assignment_type *atype = dest;
	ASSERTeq(dest_size, sizeof(enum pobj_header_type));

	if (strcmp(vstr, "global") == 0) {
		*atype = POBJ_ARENA_ASSIGNMENT_GLOBAL;
	} else if (strcmp(vstr, "thread") == 0) {
		*atype = POBJ_ARENA_ASSIGNMENT_THREAD_KEY;
	} else {
...
static const struct ctl_argument CTL_ARG(arena_assignment_type) = {
	.dest_size = sizeof(enum pobj_arena_assignment_type),
	.parsers = {
		CTL_ARG_PARSER(enum pobj_arena_assignment_type,
			arena_assignment_type_parser),
		CTL_ARG_PARSER_END
	}
};

static const struct ctl_node CTL_NODE(heap_global)[] = {
	CTL_LEAF_RW(arena_assignment_type),
...
{
	CTL_REGISTER_MODULE(NULL, heap_global);
}
