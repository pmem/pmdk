	...
	enum ctl_query_source source, void *arg, struct ctl_indexes *indexes)
{
	...
	if (ret) {
		CORE_LOG_ERROR("cannot change max arena number");
		return -1;
	}
	...
	enum ctl_query_source source, void *arg, struct ctl_indexes *indexes)
{
	...
	if (arena_id < 1 || arena_id > narenas) {
		CORE_LOG_ERROR("arena id outside of the allowed range: <1,%u>",
			narenas);
		errno = ERANGE;
		return -1;
	}
		...
		void *arg, struct ctl_indexes *indexes)
{
	...
	if (arg_in != 0 && arg_in != 1) {
		CORE_LOG_ERROR("incorrect arena state, must be 0 or 1");
		return -1;
	}
	...
	enum ctl_query_source source, void *arg, struct ctl_indexes *indexes)
{
	...
	if (size == 0) {
		CORE_LOG_ERROR("number of default arenas can't be 0");
		return -1;
	}
