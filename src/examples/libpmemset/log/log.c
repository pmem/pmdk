...
struct header {
	size_t last;
	char unused[4092 - sizeof(size_t)];
};
...
assemble_path(const char *dir, const char *name, unsigned partNum)
{
	...
	if (len > LOG_NAME_LEN) {
		fprintf(stderr, "log name: %s is t0o long ", name);
		return NULL;
	}
...
part_create(struct pmemset *set, const char *dir, const char *name, bool create,
	int partNum, struct pmemset_sds *sds,
	struct pmemset_part_descriptor *desc)
{
	...
	if (create) {
		if (pmemset_source_from_file(&src, path) == 0) {
			fprintf(stderr,
				"log: %s, allready exists, please delete it before continuing\n",
				path);
