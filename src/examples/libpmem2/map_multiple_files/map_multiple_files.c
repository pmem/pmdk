 ...
 */
static int
files_check_same_align(struct file_dsc **fdsc, size_t nfiles)
{
	size_t alignment, nalignment;
	pmem2_source_alignment((*fdsc)[0].src, &alignment);

	for (int n = 1; n < nfiles; n++) {
		pmem2_source_alignment((*fdsc)[n].src, &nalignment);
 ...
 */
static int
files_check_same_memset(struct file_dsc *fdsc, size_t nfiles,
		pmem2_memset_fn memset_fn)
{
	for (int n = 0; n < nfiles; n++) {
		if (memset_fn != pmem2_get_memset_fn(fdsc[n].map)) {
			fprintf(stderr,
				"usage: files must have the same file systems\n");
			return 1;
		}
	}
