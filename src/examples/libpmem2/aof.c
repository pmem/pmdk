...
aof_rewind(struct aof *ctx)
{
	...
	aof_header_update(ctx, -ctx->header->end_offset + sizeof(*ctx->header));
...
/*
 * vec_init -- allocate and fill aof_vec structure
 */
static int
vec_init(struct aof_vec *vec, uint8_t *addr, size_t aof_vcnt)
{
...
aof_appendv(struct aof *ctx, struct aof_vec *aofv, size_t aofvcnt)
{
	...
	for (int i = 0; i < aofvcnt; ++i) {
		if ((void *)((uintptr_t)addr + aofv->len) > ctx->end_ptr) {
			fprintf(stderr,
		...
		total_len += aofv->len;
		addr = (void *)((uintptr_t)addr + aofv->len);
		aofv++;
	}
...
main(int argc, char *argv[])
{
	...
	while (argc > 0) {
		if (strcmp(argv[0], "append") == 0 && argc > 1) {
			...
			ret = vec_init(vec, addr, aof_vcnt);
			if (ret) {
				free(vec);
				goto err_out;
			}
