...
/*
 * pool_check_init_and_check -- verifies various invariants about the pool and
 * returns its state.
 * If the device state of the pool is initialized if needed.
 */
static enum pool_state
pool_check_init_and_check(struct pool *pool)
{
...
{
	enum pool_state state = pool_check_init_and_check(pool);
