...
void pmemset_config_new(struct pmemset_config **config);
 ...
 */
typedef void pmemset_event_callback(struct pmemset *set,
 ...
 */
void pmemset_source_from_external(struct pmemset_source **source,
	struct pmem2_source *ext_source);

#ifdef WIN32
void pmemset_source_from_wfile(struct pmemset_source **source,
	const wchar *file);
void pmemset_source_from_wfile_params(struct pmemset_source **source,
	const wchar *file);
#endif
void pmemset_source_from_file(struct pmemset_source **source,
	const char *file);

void pmemset_source_from_temporary(struct pmemset_source **source,
	const char *dir);
 ...
 * source.
 */
void pmemset_source_fallocate(struct pmemset_source *source, int flags,
 ...
 */
void pmemset_new(struct pmemset **set, struct pmemset_config *config);
...
/*
 * Adds a new part to the set, creating a mapping at a given offset and length
 * in the file.
 * The part is not directly accessible until it's finalized.
 */
void pmemset_part_descriptor_new(struct pmemset_part_descriptor **part,
	struct pmemset *set,
	struct pmemset_source *source,
	size_t offset, size_t length);

/*
 * Does a memcpy from a part descriptor into the destination buffer,
 * returns an error if there's a bad block on the region.
 *
 * These can be used to fix bad blocks and safely read/write
 * sds and header data.
 */
size_t pmemset_part_descriptor_pread_mcsafe(
	struct pmemset_part_descriptor *part,
	void *dst, size_t size, size_t offset);

size_t pmemset_part_descriptor_pwrite_mcsafe(
	struct pmemset_part_descriptor *part,
	void *dst, size_t size, size_t offset);
...
/*
 * Finalizes part creation. Performs all additional checks and performs off all
 * relevant part-related events (add/remove and bad blocks).
 *
 ...
 */
void pmemset_part_finalize(struct pmemset *set,
 ...
 */
void pmemset_remove_part(struct pmemset *set, struct pmemset_part **part);
 ...
 */
void pmemset_remove_range(struct pmemset *set, void *addr, size_t length);
 ...
 * should be the same).
 */
void *pmemset_part_address(struct pmemset_part *part);
size_t pmemset_part_length(struct pmemset_part *part);

void pmemset_part_first(struct pmemset *set, struct pmemset_part **part);
void pmemset_part_next(struct pmemset *set, struct pmemset_part **part);
void pmemset_part_by_address(struct pmemset *set, struct pmemset_part **part,
	void *addr);
/*
 * Parts are owned by the set, not the caller. They need to be dropped when no
 * longer needed.
 */
void pmemset_part_drop(struct pmemset_part *part);
