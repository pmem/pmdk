...
list_add(struct pool_layout *pool, uint64_t key, uint64_t value)
{
	 ...
	 * an allocated noid is to be inserted.
	 */
...
list_check(struct pool_layout *pool)
{
	...
	char *c = malloc(pool->hdr.used_entries);
...
{
	fprintf(stderr, "usage: %s file add key value\n", name);
	fprintf(stderr, "       %s file print\n", name);
	fprintf(stderr, "       %s file check\n", name);
