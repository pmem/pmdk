 ...
 * When all is done (or at some picked checkpoint), program drains all previous
...
{
	char *elem = (char *)((uintptr_t)ctx->header + sizeof(*ctx->header));
	size_t total_len = (size_t)((uintptr_t)ctx->tail - (uintptr_t)elem);
	fwrite(elem, sizeof(char), total_len, stdout);
	printf("\n");
