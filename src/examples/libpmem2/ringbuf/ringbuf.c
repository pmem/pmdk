 ...
 */
static int
ringbuf_data_is_initialized(struct ringbuf_data *rbuf_data)
{
 ...
 */
static uint64_t
ringbuf_entry_size(struct ringbuf *rbuf)
{
...
ringbuf_data_entry_get(struct ringbuf_data *rbuf, size_t pos)
{
 ...
 */
static void
ringbuf_load_position(struct ringbuf *rbuf, uint64_t *read, uint64_t *write)
{
	uint64_t w;
	uint64_t r;
 ...
 */
static int
ringbuf_enqueue(struct ringbuf *rbuf, char *src)
{
	uint64_t r;
	uint64_t w;
...
/*
 * ringbuf_dequeue -- atomically removes one entry entry from the ring buffer.
...
thread_consumer(void *arg)
{
	...
	for (int i = 0; i < targs->nops; ++i) {
		...
		while (ringbuf_dequeue(rbuf, src) != 0)
			;
	}
...
thread_producer(void *arg)
{
	...
	memset(dst, 0xc, entry_size);

	for (int i = 0; i < targs->nops; ++i) {
		/* busy loop is intentional, avoids coordination overhead */
		while (ringbuf_enqueue(rbuf, dst) != 0)
			;
	}
...
main(int argc, char *argv[])
{
	...
	if (argc != 3) {
		fprintf(stderr, "usage: %s file nops\n", argv[0]);
		exit(1);
	}

	size_t nops = atoll(argv[2]);
	if (nops == 0) {
