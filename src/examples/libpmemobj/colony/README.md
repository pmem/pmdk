...
The colony consists of:
	- a doubly linked list of memory blocks the array that stores items and
	skipfields (Bentley pattern) to "jump over" removed items and vacants
	(there is no need to reallocate upon insertion or removal)
...
To remove a single item from the colony run the following command:
```
...
To remove items from the range (if there are) from the colony run the following
command:
```
