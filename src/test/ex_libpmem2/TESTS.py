...
class EX_LIBPMEM2(t.Test):
    ...
    def get_path(self, ctx, file_name, create_file_fn, file_size):
        ...
        filepath = ctx.create_non_zero_file(file_size,
...
class TEST0(EX_LIBPMEM2):
    ...
    def run(self, ctx):
        ...
        file_path = ctx.create_non_zero_file(
            self.file_size, Path(ctx.testdir, 'testfile0'))
...
class TEST4(EX_LIBPMEM2):
    ...
    def run(self, ctx):
        test_path = futils.get_examples_dir(ctx)
        file_path = ctx.create_holey_file(
            16 * t.MiB, Path(ctx.testdir, 'testfile0'))
        ...
        ctx.exec(binary_path, file_path, *args, log_file='out4.log')
