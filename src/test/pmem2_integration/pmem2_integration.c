...
/*
 * test_deep_sync_null_map -- try deep_sync for null map
 */
static int
test_deep_sync_null_map(const struct test_case *tc, int argc, char *argv[])
{
	char *file = argv[0];
	int fd = OPEN(file, O_RDWR);
	...
	ret = pmem2_deep_sync(NULL, addr, len);
...
static struct test_case test_cases[] = {
	...
	TEST_CASE(test_deep_sync_null_map),
