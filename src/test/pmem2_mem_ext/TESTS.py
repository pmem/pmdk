...
test_cases = \
    [
        # TEST0 - granularity PAGE/CACHELINE,       SSE2
        PAGE_CACHELINE_SMALL,

        # TEST1 - granularity BYTE,                 SSE2
        BYTE_MATCH_SMALL,

        # TEST2 - granularity PAGE/CACHELINE,       AVX512F
        PAGE_CACHELINE_SMALL,

        # TEST3 - granularity BYTE,                 AVX512F
        BYTE_MATCH_SMALL,

        # TEST4 - granularity PAGE/CACHELINE,       AVX
        PAGE_CACHELINE_SMALL,

        # TEST5 - granularity BYTE,                 AVX
        BYTE_MATCH_SMALL,

        # TEST6 - granularity PAGE/CACHELINE,       SSE
        PAGE_CACHELINE_BIG,

        # TEST7 - granularity BYTE,                 SSE
        BYTE_MATCH_BIG,

        # TEST8 - granularity PAGE/CACHELINE,       AVX512F
        PAGE_CACHELINE_BIG,

        # TEST9 - granularity BYTE,                 AVX512F
        BYTE_MATCH_BIG,

        # TEST10 - granularity PAGE/CACHELINE,      AVX
        PAGE_CACHELINE_BIG,

        # TEST11 - granularity BYTE,                AVX
        BYTE_MATCH_BIG,

        # TEST12 - PMEM_NO_MOVNT
        [[NO_FLAGS, [1024, "t"]]],

        # TEST13 - PMEM_NO_MOVNT, PMEM_NO_GENERIC_MEMCPY
        [[NO_FLAGS, [1024, "t"]]]
    ]
...
class Pmem2MemExt(t.Test):
    ...
    # to predict usage of temporal stores. This value is override is some tets
    ...
    def setup(self, ctx):
        ...
        self.available_arch = ret.returncode

    def check_arch(self):
        if "PMEM_AVX" in self.envs and self.available_arch < 2:
            raise futils.Skip("SKIP: AVX unavailable")
        if "PMEM_AVX512F" in self.envs and self.available_arch < 3:
            raise futils.Skip("SKIP: AVX512F unavailable")

    def check_log(self, ctx, match, type, flag):
        pattern = "Intrinsic function: "
        with open(os.path.join(self.cwd, self.pmem2_log), 'r') as f:
            str_val = f.read()

            # count intrinsic function, only one log should occurr at the time
            count = str_val.count(pattern)
            if count != 1:
                sys.exit("Wrong number of pattern occurrences: "
                         + str(count) + ". One expected")

            # count full match
            pattern += str(match)
            count = str_val.count(pattern)
            if count < 1:
                sys.exit("Cannot find pattern: " + pattern +
                         "\nType: " + type +
                         "\nFlag id: " + str(flag))
    ...
    def create_match(self, oper, store_type):
        ...
        if oper == "C" or oper == "M":
            ...
            sys.exit("Operation: " + oper + "not supported")
    ...
    def run(self, ctx):
        ...
        for tc in test_cases[self.testnum]:
            for o in self.oper:
                flag_id = tc[0]
                size = tc[1][0]
                store_type = tc[1][1]
