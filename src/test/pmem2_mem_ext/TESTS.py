...
class Pmem2MemExt(t.Test):
    ...
    def check_log(self, ctx, match, type, flag):
        ...
        with open(os.path.join(self.cwd, self.pmem2_log), 'r') as f:
            ...
            # count intrinsic/general function,
            # only one log should occurr at the time
            count = []
            for n, p in enumerate(pattern):
                count.append(str_val.count(pattern[n]))
            ...
            # select and count full match
            for n, c in enumerate(count):
                if c:
                    pattern = pattern[n] + str(match)
