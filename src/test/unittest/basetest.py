...
class Test(BaseTest):
    ...
    def setup(self, ctx):
        ...
        # disable shutdown state control for non-pmem (Page granurality)
        if isinstance(ctx.granularity, Page):
            env = dict()
            pmemobj_conf = os.getenv('PMEMOBJ_CONF')
            if pmemobj_conf is None or (not
