...
if sys.platform == 'win32':
    ...
    def _get_dimms_from_disk_no(disk_no):
        """
        Get dimms from device ID (disk number) using powershell cmdlets
        """
        ...
        if proc.returncode:
            raise futils.Fail('The {} disk number could not be found'
                              .format(proc.stdout))

        dimms = proc.stdout.strip().strip(r'{}').split(',')
        dimms = [d for d in dimms if d]  # remove empty strings
        if not dimms:
            raise futils.Fail('No underlying dimms found for disk number {}'
                              .format(disk_no))
        return dimms
...
def _get_dev_from_testdir_linux(testdir):
    ...
    try:
        ...
        #
        # device is a first word in second line
        try:
...
class USCTool(abc.ABC):
    """
    Abstract class implementing interface necessary to serve as an
    ...
    def inject_usc(dimm):
        """
        Inject unsafe shutdown into dimm.
