...
do_ctl_snapshots_cache_size(PMEMobjpool *pop)
{
	...
	long long cache_size = 0x7FFFFBB4;
	...
	/* change value of cache size to make sure ctl_get works correctly */
	cache_size = 0;
	ret = pmemobj_ctl_get(pop, "tx.cache.size",
					&cache_size);
	UT_ASSERTeq(ret, 0);
	UT_ASSERTeq(cache_size, 0x7FFFFBB4);
	...
	ret = pmemobj_ctl_get(pop, "tx.cache.size",
					&cache_size);
	UT_ASSERTeq(ret, 0);
	UT_ASSERTeq(cache_size, (ssize_t)PMEMOBJ_MAX_ALLOC_SIZE - 1);
