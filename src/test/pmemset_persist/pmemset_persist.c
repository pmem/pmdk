...
{
	if (argc < 1)
		UT_FATAL("usage: test_persist_multiple_parts <path>");
 ...
 * this test should failed under pmemcheck
 */
static int
 ...
 * and drain, do not flush 1/2 of second map - should failed
 */
static int
		...
		char *argv[])
{
	...
	struct pmemset_config *cfg;
	size_t first_part_size = 64 * 1024;
	size_t second_part_size = 32 * 1024;
	struct pmemset_part_descriptor first_desc;
	struct pmemset_part_descriptor second_desc;
