 ...
 * invalid uuid in a transaction where pmemobj_tx_set_failure_event
 * was called.
 */
static void
do_tx_free_wrong_uuid_abort_on_failure(PMEMobjpool *pop)
{
	...
	/* pmemobj_tx_set_failure_event is called */
	TX_BEGIN(pop) {
		pmemobj_tx_set_failure_event(POBJ_TX_FAILURE_RETURN);

		UT_ASSERTeq(pmemobj_tx_get_failure_event(),
	...
	/* pmemobj_tx_set_failure_event is called in outer tx */
	TX_BEGIN(pop) {
		pmemobj_tx_set_failure_event(POBJ_TX_FAILURE_RETURN);
		TX_BEGIN(pop) {
			UT_ASSERTeq(pmemobj_tx_get_failure_event(),
	...
	/* pmemobj_tx_set_failure_event is called in neighbour tx */
	TX_BEGIN(pop) {
		TX_BEGIN(pop) {
			pmemobj_tx_set_failure_event(POBJ_TX_FAILURE_RETURN);
			ret = pmemobj_tx_free(oid);
	...
	TX_BEGIN(pop) {
		pmemobj_tx_set_failure_event(POBJ_TX_FAILURE_RETURN);
		TX_BEGIN(pop) {
			pmemobj_tx_set_failure_event(POBJ_TX_FAILURE_ABORT);
			UT_ASSERTeq(pmemobj_tx_get_failure_event(),
				POBJ_TX_FAILURE_ABORT);
		} TX_ONCOMMIT {
			UT_ASSERTeq(ret, EINVAL);
