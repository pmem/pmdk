...
{
	/* fd == 99 - did not found any matching device */
	{ {0, 0} },
	/* fd % 50 == 1 - test #0 - no bad blocks */
	{ {0, 0} },
	/* fd % 50 == 2 - test #1 - 1 HW bad block */
	{ {1, 1}, {0, 0} },
	/* fd % 50 == 3 - test #2 - 6 HW bad blocks */
	{ {4, 10}, {16, 10}, {28, 2}, {32, 4}, {40, 4}, {50, 2}, {0, 0} },
	/* fd % 50 == 4 - test #3 - 7 HW bad blocks */
...
{
	/* fd == 99 - test_basic - did not found any matching device */
	{ {0, 0, 0} },
	/* fd % 50 == 1 - test #0 - no bad blocks */
	{ {0, 0, 0} },
	/* fd % 50 == 2 - test #1 */
...
get_next_badblock(int fd, unsigned *i_bb)
{
	...
	if (fd >= FD_REG_FILE && fd < FD_CHR_DEV)
		/* regular file */
...
main(int argc, char *argv[])
{
	...
	/*
	 *        fd ==  99 - regular file, did not found any matching device
	 * 100 <= fd <  200 - regular file
	 * 200 <= fd <  300 - character device
	 *        fd == 300 - directory
	 *        fd == 400 - block device
	 *
	 * fd % 100 <  50 - namespace mode
	 * fd % 100 >= 51 - region mode
	 */
