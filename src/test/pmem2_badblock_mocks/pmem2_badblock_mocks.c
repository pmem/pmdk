...
"   bad_blocks_set :     1-"TESTS_NUMBER_STR"\n\n"
...
parse_arguments(int argc, char *argv[], int *test, test_fn **test_func)
{
	if (argc < (ARG_NUMBER - 1) || argc > ARG_NUMBER) {
		UT_OUT(USAGE_MSG);
		if (argc > ARG_NUMBER)
			UT_FATAL("Error: too many arguments");
		else
			UT_FATAL("Error: missing required argument(s)");
	}
	...
	*test_func = NULL;
	...
	if (strcmp(test_case, "test_basic") == 0)
		...
		*test_func = test_read_clear_bb;
		...
		UT_FATAL(USAGE_MSG"Error: wrong test case: %s", test_case);
	...
	if (strcmp(file_type, "reg_file") == 0)
		...
		*test |= FD_BLK_DEV;
		...
		UT_FATAL(USAGE_MSG"Error: wrong file type: %s", file_type);
	...
	if (strcmp(mode, "no_device") == 0)
		...
		*test |= MODE_REGION;
		...
		UT_FATAL(USAGE_MSG"Error: wrong mode: %s", mode);

	int bad_blocks_set =
		(argc == 5) ? atoi(argv[ARG_BB_SET]) : DEFAULT_TEST_NUMBER;
	if (bad_blocks_set >= 1 && bad_blocks_set <= TESTS_NUMBER)
		*test |= (bad_blocks_set - 1);
	else
		UT_FATAL(USAGE_MSG"Error: wrong bad_blocks_set: %i",
			bad_blocks_set);
}
...
main(int argc, char *argv[])
{
	...
	UT_ASSERTeq(atoi(TESTS_NUMBER_STR), TESTS_NUMBER);
