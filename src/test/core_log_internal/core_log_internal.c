...
static int
test_CORE_LOG_XYZ(const struct test_case *tc, int argc, char *argv[])
{
...
/* Test for CORE_LOG_ERROR_W_ERRNO_LAST() w/ errno */
static int
test_CORE_LOG_ERROR_W_ERRNO_LAST(const struct test_case *tc, \
	int argc, char *argv[])
{
	TEST_SETUP(CORE_LOG_UT_MESSAGE ": ");
	errno = DUMMY_ERRNO;
...
static int
test_CORE_LOG_XYZ_W_ERRNO(const struct test_case *tc,
	int argc, char *argv[])
{
