...
/*
 * pmem_log_set_treshold.c -- unit test for pmem_log_set_treshold
 */
...
static enum core_log_threshold core_tresholds[] = {
	[PMEM_LOG_THRESHOLD]		= CORE_LOG_THRESHOLD,
	[PMEM_LOG_THRESHOLD_AUX]	= CORE_LOG_THRESHOLD_AUX
};

static enum core_log_level core_levels[] = {
	[PMEM_LOG_LEVEL_HARK]		= CORE_LOG_LEVEL_HARK,
	[PMEM_LOG_LEVEL_FATAL]		= CORE_LOG_LEVEL_FATAL,
	[PMEM_LOG_LEVEL_ERROR]		= CORE_LOG_LEVEL_ERROR,
	[PMEM_LOG_LEVEL_WARNING]	= CORE_LOG_LEVEL_WARNING,
	[PMEM_LOG_LEVEL_NOTICE]		= CORE_LOG_LEVEL_NOTICE,
	[PMEM_LOG_LEVEL_INFO]		= CORE_LOG_LEVEL_INFO,
...
test_log_set_treshold_helper(int error)
{
	...
	for (enum pmem_log_threshold treshold = PMEM_LOG_THRESHOLD;
		...
		for (enum pmem_log_level level = PMEM_LOG_LEVEL_HARK;
			level <= PMEM_LOG_LEVEL_DEBUG; level++) {
			Core_log_set_treshold.exp_level = core_levels[level];
			FUNC_MOCK_RCOUNTER_SET(core_log_set_threshold,
				VALIDATED_CALL);
			int ret = pmem_log_set_threshold(treshold, level);
			if (error == NO_ERRNO) {
				UT_ASSERTeq(ret, 0);
			} else {
				UT_ASSERTeq(ret, 1);
 ...
 * - if pmempmem_log_set_treshold return 0 (no error)
...
/* Check pmem_log_set_threshold EAGAIN error handling. */
static int
...
/* Check pmem_log_set_threshold EINVAL error handling. */
static int
...
{
	START(argc, argv, "pmem_log_set_treshold");
