		...
		char *argv[])
{
	...
	ret = pmemset_set_contiguous_part_coalescing(set, PMEMSET_COALESCING_FULL);
	UT_PMEMSET_EXPECT_RETURN(ret, 0);
	...
	/* when full coalescing is on, the parts should become one part mapping */
	pmemset_first_part_map(set, &first_pmap);
	UT_ASSERTne(first_pmap, NULL);
...
/*
 * test_full_coalescing_before_remove_part_map -- enable the part coalescing
 * feature map two parts to the pmemset. If no error returned those parts
 * should appear as single part mapping. Iterate over the set to obtain
 * coalesced part mapping and delete it.
 */
static int
test_full_coalescing_before_remove_part_map(const struct test_case *tc, int argc,
		char *argv[])
{
	if (argc < 1)
		UT_FATAL(
			"usage: test_full_coalescing_before_remove_part_map <path>");
...
/*
 * test_full_coalescing_after_remove_part_map -- map two parts to the pmemset,
 ...
 */
static int
test_full_coalescing_after_remove_part_map(const struct test_case *tc, int argc,
		char *argv[])
{
	if (argc < 1)
		UT_FATAL(
			"usage: test_full_coalescing_after_remove_part_map <path>");
...
static struct test_case test_cases[] = {
	...
	TEST_CASE(test_full_coalescing_after_remove_part_map),
};
