...
class TEST27(PMEMSET_PART):
    """
    Map two files to the pmemset, iterate over the set to get mappings'
    regions. Select a region that encrouches on both of those mappings with
    minimum size and delete all parts in this region.
    """
...
class TEST29(PMEMSET_PART):
    """
    Create two coalesced mapping composed of three parts, iterate over the set
    to get mapping's region. Select a region that encrouches only on the part
    situated in the middle of the coalesced mapping and delete part in
    this region.
    """
