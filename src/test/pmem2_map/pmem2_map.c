...
/*
 * test_map_addr_hint_valid - map a file when addr is initialized
 * and address type request PMEM2_ADDRESS_HINT is used
 */
static int
test_map_addr_hint_valid(const struct test_case *tc,
		int argc, char *argv[])
{
	if (argc < 2)
		UT_FATAL("usage: test_map_addr_hint_valid <file> <size>");
	...
	/* init proper addr and set address type request PMEM2_ADDRESS_HINT */
	pmem2_source_alignment(&src, &alignment);
	addr = (void *)alignment;
	pmem2_config_set_address(&cfg, addr, PMEM2_ADDRESS_HINT);
