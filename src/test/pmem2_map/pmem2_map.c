...
/*
 * prepare_config_with_addr -- extended version of ut_prepare_config -
...
{
	ut_prepare_config(cfg, src, fh, FH_FD, file, length, offset,
					access);
	cfg->addr = addr;
...
test_map_rdrw_file(const struct test_case *tc, int argc, char *argv[])
{
	...
	struct pmem2_source *src;
	struct FHandle *fh;
	ut_prepare_config(&cfg, &src, &fh, FH_FD, file, 0, 0, FH_RDWR);
...
test_map_rdonly_file(const struct test_case *tc, int argc, char *argv[])
{
	...
	struct pmem2_source *src;
	struct FHandle *fh;
	ut_prepare_config(&cfg, &src, &fh, FH_FD, file, 0, 0, FH_READ);
...
test_map_invalid_ranges(const struct test_case *tc, int argc, char *argv[])
{
	...
	ut_prepare_config(&cfg, &src, &fh, FH_FD, file, size2, offset,
					FH_RDWR);
	ret = pmem2_map(&cfg, src, &map);
	...
	offset = size * 2;
	ut_prepare_config(&cfg, &src, &fh, FH_FD, file, 0, offset,
					FH_RDWR);
	ret = pmem2_map(&cfg, src, &map);
	UT_PMEM2_EXPECT_RETURN(ret, PMEM2_E_MAP_RANGE);
	PMEM2_SOURCE_DELETE(&src);
...
test_map_invalid_alignment(const struct test_case *tc, int argc, char *argv[])
{
	...
	ut_prepare_config(&cfg, &src, &fh, FH_FD, file, length,
					KILOBYTE, FH_RDWR);
	int ret = pmem2_map(&cfg, src, &map);
...
test_map_invalid_fd(const struct test_case *tc, int argc, char *argv[])
{
	...
	ut_prepare_config(&cfg, &src, &fh, FH_FD, file, length, 0,
					FH_RDWR);
	UT_FH_CLOSE(fh);
	int ret = pmem2_map(&cfg, src, &map);
...
test_unmap_valid(const struct test_case *tc, int argc, char *argv[])
{
	...
	ut_prepare_config(&cfg, &src, &fh, FH_FD, file, size, 0,
					FH_RDWR);
					...
					char *argv[])
{
	...
	ut_prepare_config(&cfg, &src, &fh1, FH_FD, file, 0, 0,
					FH_RDWR);
	...
	struct FHandle *fh2;
	ut_prepare_config(&cfg, &src, &fh2, FH_FD, file, 0, 0,
					FH_RDWR);
