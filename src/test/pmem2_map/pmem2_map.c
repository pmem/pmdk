			...
			struct pmem2_map **map, const char *data)
{
	...
	memcpy_fn((*map)->addr, data, strlen(data), 0);
	UT_ASSERTeq(memcmp((*map)->addr, data, strlen(data)), 0);
}
...
{
	if (argc < 1)
		UT_FATAL("usage: test_map_sharing <file>");
	...
	prepare_config(&cfg, &src, &fd, file, 0, 0, O_RDWR);
	/*
	 * PMEM2_SHARED is a default option it is set here to make the test
	 * more clear
	 */
	...
	UT_ASSERTeq(memcmp(map1->addr, word2, strlen(word2)), 0);
	UT_ASSERTne(memcmp(map1->addr, word1, strlen(word1)), 0);
...
test_map_sharing_private(const struct test_case *tc, int argc, char *argv[])
{
	...
	prepare_config(&cfg, &src, &fd, file, 0, 0, O_RDWR);
	/*
	 * map_shared is a default option it is set here to make the test
	 * more clear
	 */
	pmem2_config_set_sharing(&cfg, PMEM2_SHARED);
	...
	UT_ASSERTne(memcmp(map1->addr, word2, strlen(word2)), 0);
	UT_ASSERTeq(memcmp(map1->addr, word1, strlen(word1)), 0);
...
{
	if (argc < 1)
		UT_FATAL("usage: test_map_sharing_with_reopened_fd <file>");
