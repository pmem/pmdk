...
/*
 * test_null_addr_any - setting null addr when request type
 * PMEM2_ADDRESS_ANY is used
 */
static int
test_null_addr_any(const struct test_case *tc, int argc,
		char *argv[])
{
	struct pmem2_config cfg;
	pmem2_config_init(&cfg);
...
/*
 * test_not_null_addr_any - setting not null addr when request
 * type PMEM2_ADDRESS_ANY is used
 */
static int
test_not_null_addr_any(const struct test_case *tc, int argc,
		char *argv[])
{
	struct pmem2_config cfg;
	pmem2_config_init(&cfg);
...
static struct test_case test_cases[] = {
	...
	TEST_CASE(test_not_null_addr_any),
};
