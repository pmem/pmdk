...
/*
 * test_uninit_addr_hint - setting uninitialized addr when request
 * type PMEM2_ADDRESS_HINT is used
 */
static int
test_uninit_addr_hint(const struct test_case *tc, int argc,
		char *argv[])
{
	struct pmem2_config cfg;
	pmem2_config_init(&cfg);
...
/*
 * test_init_addr_hint - setting initialized addr when request
 * type PMEM2_ADDRESS_HINT is used
 */
static int
test_init_addr_hint(const struct test_case *tc, int argc,
		char *argv[])
{
	struct pmem2_config cfg;
	pmem2_config_init(&cfg);
...
static struct test_case test_cases[] = {
	...
	TEST_CASE(test_uninit_addr_hint),
	TEST_CASE(test_init_addr_hint),
};
