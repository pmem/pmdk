...
/*
 * test_uninit_addr_noreplace - setting uninitialized addr when request
 * type PMEM2_ADDRESS_FIXED_NOREPLACE is used
 */
static int
test_uninit_addr_noreplace(const struct test_case *tc, int argc,
		char *argv[])
{
	struct pmem2_config cfg;
	pmem2_config_init(&cfg);
...
/*
 * test_uninit_addr_any - setting uninitialized addr when request
 * type PMEM2_ADDRESS_ANY is used
 */
static int
test_uninit_addr_any(const struct test_case *tc, int argc,
		char *argv[])
{
	struct pmem2_config cfg;
	pmem2_config_init(&cfg);
...
/*
 * test_init_addr_any - setting initialized addr when request
 * type PMEM2_ADDRESS_ANY is used
 */
static int
test_init_addr_any(const struct test_case *tc, int argc,
		char *argv[])
{
	struct pmem2_config cfg;
	pmem2_config_init(&cfg);

	/* "randomly" chosen value of address */
	void *addr = (void *)(1024 * 1024);

	int ret = pmem2_config_set_address(
			&cfg, addr, PMEM2_ADDRESS_ANY);
	UT_PMEM2_EXPECT_RETURN(ret, PMEM2_E_ADDRESS_NOT_NULL);
...
static struct test_case test_cases[] = {
	...
	TEST_CASE(test_uninit_addr_noreplace),
	TEST_CASE(test_uninit_addr_any),
	TEST_CASE(test_init_addr_any),
};
