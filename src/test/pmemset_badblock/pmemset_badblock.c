...
/*
 * badblock_event_cb -- callback for badblock event
 */
static int
		...
		void *arg)
{
	if (ctx->type == PMEMSET_EVENT_BADBLOCK) {
		...
		struct badblock_event_arg *args =
				(struct badblock_event_arg *)arg;

		/*
		 * offset == 0 and length == 0, means that all bad blocks were
		 * cleared from the source
		 */
