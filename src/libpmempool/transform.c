	...
	for (unsigned r = repn; r < set->nreplicas; ++r) {
		...
		for (unsigned p = i; p < repr->nparts; ++p) {
			...
			if (result == 0) {
				...
				ERR_WO_ERRNO("some part file's path is"
						" used multiple times");
				ret = -1;
				errno = EINVAL;
				free(pathp);
				...
				ERR_WO_ERRNO(
					"comparing file inodes failed for %s"
					" and %s", path, pathp);
				ret = -1;
				free(pathp);
				goto out;
	...
	for (unsigned ri = 0; ri < set_in->nreplicas; ++ri) {
		...
		for (unsigned ro = 0; ro < set_out->nreplicas; ++ro) {
			...
			if (set_in_s->replica[ri] != UNDEF_REPLICA ||
				...
				ERR_WO_ERRNO(
					"there are more then one corresponding"
		...
		struct poolset_compare_status *set_out_s)
{
	...
	if ((set_in->options & OPTION_NOHDRS) ||
		...
		errno = EINVAL;
		ERR_WO_ERRNO(
			"the NOHDRS poolset option is not supported in "
			"local poolset files");
		return -1;
	}
		...
		unsigned flags)
{
	...
	if (operation == RM_HDRS) {
		if (!is_dry_run(flags) &&
				remove_hdrs(set_in, set_out, set_in_hs,
						flags)) {
			ERR_WO_ERRNO(
				"removing headers failed; falling back to the "
				"input poolset");
	...
	if (operation == ADD_HDRS) {
		if (!is_dry_run(flags) &&
			...
			ERR_WO_ERRNO(
				"adding headers failed; falling back to the "
				"input poolset");
			if (replica_sync(set_in, set_in_hs,
					flags | IS_TRANSFORMED)) {
				LOG(1, "falling back to the input poolset "
