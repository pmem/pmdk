...
/*
 * Core_log_function -- pointer to the logging function saved as uintptr_t to make it _Atomic,
...
core_log_init(const char *prefix)
{
	...
	while (RPMA_E_AGAIN == core_log_set_function(CORE_LOG_USE_DEFAULT_FUNCTION))
		;
}
...
#ifdef ATOMIC_OPERATIONS_SUPPORTED
	atomic_store_explicit(&Core_log_function, (uintptr_t)log_function, __ATOMIC_SEQ_CST);
	return 0;
#else
 ...
 */
int
core_log_set_threshold(enum core_log_threshold threshold, enum core_log_level level)
{
	if (threshold != CORE_LOG_THRESHOLD && threshold != CORE_LOG_THRESHOLD_AUX)
		return RPMA_E_INVAL;
...
#ifdef ATOMIC_OPERATIONS_SUPPORTED
	atomic_store_explicit(&Log_threshold[threshold], level, __ATOMIC_SEQ_CST);
	return 0;
#else
	...
	if (__sync_bool_compare_and_swap(&Core_log_threshold[threshold], level_old, level))
		return 0;
	else
		return RPMA_E_AGAIN;
#endif /* ATOMIC_OPERATIONS_SUPPORTED */
 ...
 */
int
core_log_get_threshold(enum core_log_threshold threshold, enum core_log_level *level)
{
	if (threshold != CORE_LOG_THRESHOLD && threshold != CORE_LOG_THRESHOLD_AUX)
		return RPMA_E_INVAL;
...
#ifdef ATOMIC_OPERATIONS_SUPPORTED
	*level = atomic_load_explicit(&Log_threshold[threshold], __ATOMIC_SEQ_CST);
#else
