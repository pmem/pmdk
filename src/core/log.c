...
/*
 * _core_log_get_threshold_internal -- a core_log_get_threshold variant allowing
 * not affecting the stack size of all the functions using
 * the CORE_LOG_* macros.
 */
enum core_log_level
_core_log_get_threshold_internal(enum core_log_threshold threshold)
{
	enum core_log_level level;
	(void) core_log_get_threshold(threshold, &level);
	return level;
}
	...
	const char *function_name, const char *message_format, va_list arg)
{
	 ...
	 */
	(void) core_log_get_threshold(CORE_LOG_THRESHOLD, &threshold);
	if (level > threshold)
		goto end;
