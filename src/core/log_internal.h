...
int core_log_set_threshold(enum core_log_threshold threshold, enum core_log_level level);

int core_log_get_threshold(enum core_log_threshold threshold, enum core_log_level *level);
...
#define CORE_LOG(level, format, ...) \
	do { \
		if (level <= Core_log_threshold[RPMA_LOG_THRESHOLD] && 0 != Core_log_function) { \
			((log_function *)Core_log_function)(level, __FILE__, __LINE__, \
...
#define CORE_LOG_ALWAYS(format, ...) \
	core_log_default_function(CORE_LOG_LEVEL_ALWAYS, __FILE__, __LINE__, __func__, \
					format "\n", ##__VA_ARGS__)
