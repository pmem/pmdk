...
pmem2_vm_reservation_extend(struct pmem2_vm_reservation *rsv, size_t size)
{
	...
	rsv->size += size;
	if (rsv->size > rsv->reserv_size) {
		size_t align = rsv->align;

		void *extend_addr = (char *)rsv->reserv_addr + rsv->reserv_size;
		size_t extend_size = ALIGN_UP(rsv->reserv_size + size, align) -
				rsv->reserv_size;
		...
		size_t size)
{
	...
	/* new address and size aligned to the predicted alignment */
	size_t align = rsv->align;
