// SPDX-License-Identifier: BSD-3-Clause
/* Copyright 2018-2020, Intel Corporation */

/*
 * extent_linux.c - implementation of the linux fs extent query API
 */

#include <string.h>
#include <fcntl.h>
#include <sys/ioctl.h>
#include <linux/fs.h>
#include <linux/fiemap.h>

#include "libpmem2.h"
#include "pmem2_utils.h"

#include "file.h"
#include "out.h"
#include "extent.h"
#include "alloc.h"

/*
 * pmem2_extents_common -- (internal) common part of reading extents
 *
 * Sets: struct fiemap, struct extents
 */
static int
pmem2_extents_common(int fd, struct extents *exts, struct fiemap **pfmap)
{
	LOG(3, "fd %i exts %p pfmap %p", fd, exts, pfmap);

	ASSERTne(pfmap, NULL);
	ASSERTne(exts, NULL);

	enum pmem2_file_type pmem2_type;
	os_stat_t st;

	if (os_fstat(fd, &st) < 0) {
		ERR("!fstat %d", fd);
		return PMEM2_E_ERRNO;
	}

	int ret = pmem2_get_type_from_stat(&st, &pmem2_type);
	if (ret)
		return ret;

	/* directories do not have any extents */
	if (pmem2_type == PMEM2_FTYPE_DIR) {
		ERR(
			"checking extents does not make sense in case of directories");
		return PMEM2_E_INVALID_FILE_TYPE;
	}

	if (exts->extents_count == 0) {
		LOG(10, "fd %i: block size: %li", fd, (long int)st.st_blksize);
		exts->blksize = (uint64_t)st.st_blksize;
	}

	/* devdax does not have any extents */
	if (pmem2_type == PMEM2_FTYPE_DEVDAX) {
		exts->extents_count = 0;
		return 0;
	}

	ASSERTeq(pmem2_type, PMEM2_FTYPE_REG);

	struct fiemap *fmap = pmem2_zalloc(sizeof(struct fiemap), &ret);
	if (ret)
		return ret;

	fmap->fm_start = 0;
	fmap->fm_length = (size_t)st.st_size;
	fmap->fm_flags = 0;
	fmap->fm_extent_count = 0;

	if (ioctl(fd, FS_IOC_FIEMAP, fmap) != 0) {
		ERR("!ioctl %d", fd);
		ret = PMEM2_E_ERRNO;
		goto error_free;
	}

	if (exts->extents_count == 0) {
		/* save number of extents */
		exts->extents_count = fmap->fm_mapped_extents;
		LOG(4, "fd: %i: number of extents: %u",
			fd, exts->extents_count);

	}

	*pfmap = fmap;

	return 0;

error_free:
	Free(fmap);

	return ret;
}

/*
 * pmem2_extents_count -- save number of extents of the file
 *                        in exts->extents_count and
 *                        block size in exts->blksize
 *
 * Sets:
 * - exts->extents_count - number of extents of the file
 * - exts->blksize       - block size
 */
int
pmem2_extents_count(int fd, struct extents *exts)
{
	LOG(3, "fd %i extents %p", fd, exts);

	ASSERTne(exts, NULL);
	memset(exts, 0, sizeof(*exts));

	struct fiemap *fmap = NULL;

	int ret = pmem2_extents_common(fd, exts, &fmap);

	Free(fmap);

	return ret;
}

/*
 * pmem2_extents_get -- get extents of the given file
 *
 * pmem2_extents_count() has to be called earlier
 * to know the number of file's extents
 * and the 'exts->extents' array has to be allocated
 * with 'exts->extents_count' number of elements.
 */
int
pmem2_extents_get(int fd, struct extents *exts)
{
	LOG(3, "fd %i extents %p", fd, exts);

	ASSERTne(exts, NULL);

	if (exts->extents_count == 0)
		return 0;

	ASSERTne(exts->extents, NULL);

	struct fiemap *fmap = NULL;
	int ret = pmem2_extents_common(fd, exts, &fmap);
	if (ret)
		goto error_free;

	if (exts->extents_count < fmap->fm_mapped_extents) {
		/*
		 * Number of extents has increased
		 * after pmem2_extents_count() was called,
		 * so the 'exts->extents' array is too small
		 * and it has to be reallocated to the new size.
		 */
		ret = PMEM2_E_NUMBER_OF_EXTENTS_INCREASED;
		ERR("number of extents has increased (was: %u, is: %u)",
			exts->extents_count, fmap->fm_mapped_extents);
		/*
		 * The current number of extents is returned
		 * in exts->extents_count
		 */
		exts->extents_count = fmap->fm_mapped_extents;
		goto error_free;
	}

	/* update exts->extents_count if number of extents has decreased */
	if (exts->extents_count > fmap->fm_mapped_extents)
		exts->extents_count = fmap->fm_mapped_extents;

	struct fiemap *newfmap = Realloc(fmap, sizeof(struct fiemap) +
						fmap->fm_mapped_extents *
						sizeof(struct fiemap_extent));
	if (newfmap == NULL) {
		ERR("!Realloc");
		ret = PMEM2_E_ERRNO;
		goto error_free;
	}

	fmap = newfmap;
	fmap->fm_extent_count = fmap->fm_mapped_extents;

	memset(fmap->fm_extents, 0, fmap->fm_mapped_extents *
					sizeof(struct fiemap_extent));

	if (ioctl(fd, FS_IOC_FIEMAP, fmap) != 0) {
		ERR("!ioctl %d", fd);
		ret = PMEM2_E_ERRNO;
		goto error_free;
	}

	if (fmap->fm_extent_count > 0) {
		LOG(10, "file with fd %i has %u extents:",
			fd, fmap->fm_extent_count);
	}

	unsigned e;
	for (e = 0; e < fmap->fm_extent_count; e++) {
		exts->extents[e].offset_physical =
			fmap->fm_extents[e].fe_physical;
		exts->extents[e].offset_logical =
			fmap->fm_extents[e].fe_logical;
		exts->extents[e].length =
			fmap->fm_extents[e].fe_length;

		LOG(10, "   #%u: off_phy: %lu off_log: %lu len: %lu",
			e,
			exts->extents[e].offset_physical,
			exts->extents[e].offset_logical,
			exts->extents[e].length);
	}

	ret = 0;

error_free:
	Free(fmap);

	return ret;
}
