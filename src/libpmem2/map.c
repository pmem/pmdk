...
pmem2_map_init(void)
{
	...
	ri = ravl_interval_new(&tree, mapping_min, mapping_max);
...
{
	util_rwlock_wrlock(&lock);
	int ret = ravl_interval_insert(&tree, ri, map);
	util_rwlock_unlock(&lock);
...
{
	util_rwlock_wrlock(&lock);
	int ret = ravl_interval_remove(&tree, ri, map);
	util_rwlock_unlock(&lock);
...
pmem2_map_find(const void *addr, size_t len)
{
	...
	struct pmem2_map *found_map = (struct pmem2_map *)ravl_interval_find
			(&tree, ri, &map);
	util_rwlock_unlock(&lock);
