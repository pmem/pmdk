		...
		size_t offset)
{
	...
	if (retsize == -1) {
		if (errno == EIO) {
			ERR_WO_ERRNO(
				"physical I/O error occurred on read operation, "
				"possible bad block");
		...
		size_t offset)
{
	...
	if (retsize == -1) {
		if (errno == EIO) {
			ERR_WO_ERRNO(
				"physical I/O error occurred on write operation, "
					"possible bad block");
			return PMEM2_E_IO_FAIL;
		}
...
pmem2_source_type_check_mcsafe_supp(struct pmem2_source *src)
{
	if (src->type != PMEM2_SOURCE_FD && src->type != PMEM2_SOURCE_HANDLE) {
		ERR_WO_ERRNO(
			"operation doesn't support provided source type, only "
...
pmem2_source_check_op_size(struct pmem2_source *src, size_t size, size_t offset)
{
	...
	if (size > max_size) {
		ERR_WO_ERRNO(
			"size of read %zu from offset %zu goes beyond the file "
