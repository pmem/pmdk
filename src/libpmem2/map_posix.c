		...
		const struct pmem2_config *cfg)
{
	...
	/*
	 * When kernel does not support MAP_FIXED_NOREPLACE flag we are
	 * emulating it. If kernel does not support flag and given addr is
	 * occupied, kernel chooses new addr randomly and returns it. We do not
	 * want that bevahiour, so we validate it and fail.
	 */
