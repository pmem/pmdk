...
{
	if (request_type != PMEM2_ADDRESS_ANY &&
	...
	if (request_type == PMEM2_ADDRESS_ANY && addr) {
		ERR(
			"cannot use address request type PMEM2_ADDRESS_ANY with addr being not NULL");
		return PMEM2_E_ADDRESS_NOT_NULL;
	}
