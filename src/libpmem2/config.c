...
/*
 * pmem2_config_set_address -- set addr and addr_flag in the config
 * struct
 */
int
...
{
	/*
	 * XXX: after support to PMEM2_ADDRESS_FIXED_REPLACE, it should
	 * be also considered and added to that if statement
	 */
	if (type != PMEM2_ADDRESS_ANY &&
			type != PMEM2_ADDRESS_FIXED_NOREPLACE) {
		ERR("invalid flag 0x%x", type);
		return PMEM2_E_INVALID_ADDRESS_FLAG;
	}

	/*
	 * XXX: after support to PMEM2_ADDRESS_FIXED_REPLACE, it should
	 * be also considered and validated
	 */
	if (type == PMEM2_ADDRESS_FIXED_NOREPLACE && !addr) {
		ERR("cannot use flag PMEM2_ADDRESS_FIXED_NOREPLACE with"
				" addr being NULL");
		return PMEM2_E_ADDRESS_UNINITIALIZED;
	}
