...
/*
 * pmem2_badblock_get_extents -- (internal) get file's extents
 */
static int
pmem2_badblock_get_extents(const struct pmem2_source *src,
		struct pmem2_badblock_context *tbbctx)
{
	LOG(3, "src %p tbbctx %p", src, tbbctx);

	ASSERTne(tbbctx, NULL);

	struct extents *exts = NULL;
	int ret;

	tbbctx->exts = pmem2_zalloc(sizeof(struct extents), &ret);
	if (ret)
		return ret;
	...
	*bbctx = NULL;
	...
	if (ndctl_namespace_get_mode(ndns) == NDCTL_NS_MODE_FSDAX) {
		...
		if (badblocks_get_namespace_bounds(region, ndns,
						&ns_beg, &ns_size)) {
			LOG(1, "cannot read namespace's bounds");
			ret = PMEM2_E_CANNOT_READ_BOUNDS;
			goto error_free_all;
		}
	...
	if (pmem2_type == PMEM2_FTYPE_REG) {
		...
		ret = pmem2_badblock_get_extents(src, tbbctx);
		if (ret)
			goto error_free_all;
	}
			...
			struct pmem2_badblock *bb)
{
	...
	int ret = PMEM2_E_NO_BAD_BLOCK_FOUND;
