...
badblocks_get(const char *file, struct badblocks *bbs)
{
	...
	if (ret) {
		if (ret == PMEM2_E_NO_DEVICE_FOUND) {
...
badblocks_clear(const char *file, struct badblocks *bbs)
{
	...
	ret = pmem2_badblock_context_new(&src, &bbctx);
	if (ret) {
		LOG(1, "pmem2_badblock_context_new -- %s", file);
		goto exit_close;
	}
...
badblocks_clear_all(const char *file)
{
	...
	ret = pmem2_badblock_context_new(&src, &bbctx);
	if (ret == PMEM2_E_NO_DEVICE_FOUND) {
		LOG(1, "did not found any matching device -- %s", file);
		ret = 0;
		goto exit_close;
	}
	if (ret) {
			...
			struct pmem2_badblock *bb)
{
	...
	if (!exts)
		return (*bbctx->pmem2_badblock_next_func)(bbctx, bb);
	...
	do {
		if (bbctx->exts.last_bb) {
			...
			ret = (*bbctx->pmem2_badblock_next_func)(bbctx, bb);
			if (ret)
				return ret;
		}
