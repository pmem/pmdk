		...
		struct pmemset_part_map **pmap_ptr)
{
	...
	ret = pmemset_config_event_callback(set->set_config, set, &ctx);
	if (ret)
...
pmemset_remove_range(struct pmemset *set, void *addr, size_t len)
{
	...
	int ret = pmemset_config_event_callback(set->set_config, set, &ctx);
	if (ret)
		return ret;
	...
	util_rwlock_wrlock(&set->shared_state.lock);
	ret = pmemset_iterate(set, addr, len, pmemset_remove_part_map_range_cb,
			&arg);
	util_rwlock_unlock(&set->shared_state.lock);
