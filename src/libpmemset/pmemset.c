...
pmemset_delete_all_part_maps_ravl_cb(void *data, void *arg)
{
	...
	int ret = pmemset_part_map_delete(pmap);
	ASSERTeq(ret, 0);
}
...
pmemset_delete(struct pmemset **set)
{
	...
	ravl_interval_delete_cb((*set)->part_map_tree,
			pmemset_delete_all_part_maps_ravl_cb, NULL);
		...
		struct pmemset_part_descriptor *desc)
{
	...
	int ret;
	/* if part size == 0 then then part size == file size */
	if (!part_size) {
		ret = pmem2_source_size(pmem2_src, &part_size);
		if (ret)
			return ret;
	...
	bool coalesced = true;
	struct pmemset_part_map *pmap = set->previous_pmap;
	/* if there are no parts in pmemset, then new part can't be coalesced */
	enum pmemset_coalescing coalescing = (pmap) ? set->part_coalescing :
			PMEMSET_COALESCING_NONE;
	switch (coalescing) {
		case PMEMSET_COALESCING_OPPORTUNISTIC:
		case PMEMSET_COALESCING_FULL:
			ret = pmemset_part_map_extend_end(pmap, part_size);

			if (!ret || coalescing == PMEMSET_COALESCING_FULL)
				break;
...
/*
 * pmemset_part_map_remove_range_cb -- wrapper for removing part map range on
 *                                     each iteration
 */
static int
pmemset_part_map_remove_range_cb(struct pmemset *set,
		struct pmemset_part_map *pmap, void *arg)
{
	...
	return pmemset_part_map_remove_range(pmap, offset, adjusted_size);
}
...
pmemset_remove_range(struct pmemset *set, void *addr, size_t len)
{
	...
	return pmemset_iterate(set, addr, len, pmemset_part_map_remove_range_cb,
			&arg);
}
