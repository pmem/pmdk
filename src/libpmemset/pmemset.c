...
pmemset_remove_part_map(struct pmemset *set, struct pmemset_part_map **pmap_ptr)
{
	...
	if (pmap->refcount > 1) {
		ERR(
			...
			"part map should only be referenced once and by the thread deleting it",
				pmap->refcount);
		return PMEMSET_E_PART_MAP_REFCOUNT_TOO_BIG;
	}
