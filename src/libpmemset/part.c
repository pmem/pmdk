	...
	*part_map = NULL;
	...
	if (ret) {
		if (ret == PMEM2_E_MAPPING_EXISTS)
			ret = PMEMSET_E_CANNOT_COALESCE_PART_MAPS;
		Free(pmap);
		goto err_cfg_delete;
	}
...
pmemset_part_map_delete(struct pmemset_part_map **part_map)
{
	...
	/* find mapping lying at the end of the reservation */
	...
	if (rsv_addr == pmem2_map_addr && rsv_size == pmem2_map_size) {
		/* mapping covers whole vm reservation */
		pmem2_vm_reservation_delete(&pmem2_reserv);
		Free(pmap);
	} else {
		/* mapping covers part of the vm reservation */
