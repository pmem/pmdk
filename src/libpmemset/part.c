...
/*
 * pmemset_part_file_truncate -- truncate part file if source
 * is from a temp file
 */
static int
pmemset_part_file_truncate(struct pmemset_part *part, size_t source_size)
{
		...
		enum pmemset_coalescing part_coalescing)
{
	*part_map = NULL;
	...
	ret = pmemset_part_file_truncate(part, source_size);
	if (ret) {
