        ...
        if found:
            ...
            if code[-1] == '\\': # template's type of ending a line
                if code[-2] == ';': # a single line call can be extracted
                    ...
                    print(f'Multiline calls in templates are not supported: {line}')
                    continue
            elif code[-1] != ';':
...
def main():
    ...
    for func in ['CORE_LOG_ERROR_LAST', 'ERR_WO_ERRNO', 'CORE_LOG_WARNING', 'CORE_LOG_ERROR', 'CORE_LOG_FATAL']:
        calls = extract_all_calls(func)
        global_total += len(calls)
        calls = calls_process(calls)
        generate_func(func, calls)

    for func in ['CORE_LOG_ERROR_W_ERRNO_LAST', 'ERR_W_ERRNO', 'CORE_LOG_WARNING_W_ERRNO', 'CORE_LOG_ERROR_W_ERRNO', 'CORE_LOG_FATAL_W_ERRNO']:
        calls = extract_all_calls(func)
        global_total += len(calls)
        calls = calls_process(calls)
