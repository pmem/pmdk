...
function test_built_packages {
	...
	python3 $GHA_WORKDIR/scripts/test-build-packages.py -r $(pwd)
}

# install_packages -- install built packages.
function install_packages {
	echo "********** install packages **********"
	sudo python3 $GHA_WORKDIR/scripts/install-packages.py -r $(pwd)
}
...
function test_installed_packages {
	...
	python3 $GHA_WORKDIR/scripts/test-packages-installation.py -r $(pwd)
}
