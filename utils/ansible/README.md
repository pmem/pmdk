...
environment on a real hardware to be able to build a PMDK project on it or
use the node as a self-hosted runner for the pmem/pmdk repository.
...
# Installing a GitHub Action self-hosted runner
Having a server with persistent memory and a CI environment already installed,
the sequence of commands below sets up the server as a self-hosted runner
...
[Create self-hosted runner](https://github.com/pmem/pmdk/settings/actions/runners/new)
.

**Note**: In case of any problems, please refer to the
