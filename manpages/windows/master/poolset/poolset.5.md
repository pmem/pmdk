...
written to local  pool replicas, thereby providing a backup
...
with the **pmemobj_createU**()/**pmemobj_createW**(), function or by using the **pmempool**(1) utility.
...
the replicated pool set, the *path* argument passed to **pmemobj_createU**()/**pmemobj_createW**(),
...
pool set, the *path* argument passed to **pmemobj_openU**()/**pmemobj_openW**() must not
...
part size defined in *set* file **pmemobj_openU**()/**pmemobj_openW**() returns NULL and sets
*errno* appropriately.
