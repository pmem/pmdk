...
lost in case of *a power-failure*. Failure of this mechanism is possible, in that
case system will be shutdown unsafely with the possibility of data loss. This
behavior can be discovered by checking the *unsafe shutdown counter*. For more
information this mechanism, see **libpmem2_unsafe_shutdown**(7).
...
terminate the mapping process. This can be changed, the user can provide a list of
acceptable part states using **pmemset_config_set_acceptable_states**(3) function.
Part *shutdown states* belonging to this list will not cause an error.
