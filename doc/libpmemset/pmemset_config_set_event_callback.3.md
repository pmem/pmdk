...
[comment]: <> (pmemset_config_set_event_callback.3 -- man page for pmemset_config_set_set_event_callback)
...
The callback will be called by pmemset each time when an event will occurs.
Events are only fired during the user's calls of the libpmemset methods.
...
The library user must guarantee this by not having multiple threads mutating the same region on the set.
Once the function exists ctx and its data are invalid.
