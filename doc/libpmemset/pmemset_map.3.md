...
bad blocks are immediately cleared in event callback function, then a **PMEMSET_EVENT_BADBLOCK** event is
fired, with bad block offset and size equal 0. For bad blocks clearing, see **pmemset_badblock_clear**(3).
