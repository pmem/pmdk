...
It means that each part will have the same granularity defined in advanced using **pmemset_config_set_require_store_granularity()**.

Regardless od the type od the pmemset source, the **libpmemset(7)** library is using **libpmem2(7)** API to map parts.
...
For more information please read **pmem2_config_set_require_store_granularity(3)** mand page
