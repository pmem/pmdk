...
* **PMEMSET_SOURCE_FILE_CREATE_MODE(mode)** - the *mode* argument specifies the file mode to be
        applied when a new file is created. This macro should be apply using bitwise OR oparator with the above create *flags*.
        The following *mode* values are available:
        * **PMEMSET_SOURCE_FILE_RWXU_MODE** - user has read, write, and execute permission
        * **PMEMSET_SOURCE_FILE_RUSR_MODE** - user has read pesmission
        ...
        * **PMEMSET_SOURCE_FILE_RGRP_MODE** - group has read pesmission
        * **PMEMSET_SOURCE_FILE_WGRP_MODE** - group has write pesmission
        * **PMEMSET_SOURCE_FILE_XGRP_MODE** - group has exacute pesmission
...
If *PMEMSET_SOURCE_FILE_CREATE_MODE(mode)* if not specified then the default file mode is set
...
>NOTE: The *mode* argument is not supporter on Windows.
