...
part mapping. The behavior of part mapping can be changed by setting one of possible values in pmemset:
...
With contiguous part coalescing feature enabled, **pmemset_part_map**() function tries to map each
new part at the virtual memory region that is situated right after the previous mapped part memory range.
There's no guarantee that the desired memory region is unoccupied. Mapping parts contiguously allows
modifying the virtual address space of multiple parts with one operation using for example **memset**(3).
