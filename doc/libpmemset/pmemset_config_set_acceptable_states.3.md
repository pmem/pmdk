...
* **PMEMSET_PART_STATE_OK_BUT_ALREADY_OPEN** - the pool is internally consistent, but
...
was not closed cleanly. It is an expected state when creating multiple mappings from
the same *pmemset_source*.

* **PMEMSET_PART_STATE_CORRUPTED** - the pool can contain invalid data as a result of hardware
failure. Reading the pool is unsafe. Application might need to perform consistency checking and
