...
\<poolset-file-name\>_r\<replica-number\>_p\<part-number\>_badblocks.txt where
\<poolset-file-name\> is the name of the poolset file. These recovery files
are automatically removed if the sync operation finishes successfully.
...
blocks was not recovered (not copied from a healthy replica).
...
will be read from the saved recovery files).
