...
In case of a remote replica, the *REPLICA* keyword has to be followed by an address of a remote host (in the format recognized by the ssh command)
and a relative path to a remote pool set file:

```
REPLICA [user@]hostname remote-pool-set-file
```
