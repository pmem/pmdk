...
The **rpmem_drain**() can be replaced with **rpmem_persist**() at no cost except
the case where the work queue has a single free space where **rpmem_persist**()
requires two free spaces in the work queue. It forces to wait for proceeding
tasks to complete prior to schedule **rpmem_persist**() where **rpmem_drain**()
requires a single free space in the work queue so in this case no preliminary
waiting is required.
