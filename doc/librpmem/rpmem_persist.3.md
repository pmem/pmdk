...
int rpmem_deep_flush(RPMEMpool *rpp, size_t offset,
	size_t length, unsigned lane);
...
The **rpmem_deep_flush**() function works in the same way as
**rpmem_persist**(3) function, but additionaly it performs WPQ flush on the
underlying NVDIMM(s). Please see **pmem_deep_flush**(3) for details.
