...
**pmemobj_tx_begin**() function handles errors differently, based on **setjmp**
macro enablement. If **setjmp** is not set, then after transaction failure
(i.e. any function in transaction calls abort), the **errno** is set
appropriately, transaction is aborted and the stage is changed to *TX_STAGE_ONABORT*.
With set **setjmp** macro, function will do the same and after those steps
**longjmp** is called to get back to **env** saved.
...
**pmemobj_tx_lock**() when *flags* is equals to **POBJ_XLOCK_NO_ABORT**.
When *flags* is equals to 0 the stageis changed to **TX_SATGE_ONABORT**
...
+ **POBJ_XLOG_APPEND_BUFFER_NO_ABORT** - if the function ends incorrectly,
return error number instead of aborting the transaction.
...
when **PMEMOBJ_XLOG_APPEND_BUFFER_NO_ABORT** flag is enabled error number
is returned, when flag is disabled the transaction is aborted, the stage
is changed to *TX_STAGE_ONABORT* and *errno* is set appropriately.
