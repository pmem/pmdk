...
_UWFUNCR2(int, pmemobj_ctl_set, PMEMobjpool *pop, *name, void *arg,
	=q= (EXPERIMENTAL)=e=)
_UWFUNCR2(int, pmemobj_ctl_run, PMEMobjpool *pop, *name, void *arg,
	=q= (EXPERIMENTAL)=e=)
```
...
The _UW(pmemobj_ctl_get), _UW(pmemobj_ctl_set) and _UW(pmemobj_ctl_run)
...
trigger a function or a combination of these operations.

The entry points are listed in the following format:

name | r(ead)w(rite)t(rigger) | global/- | read argument type | write argument type | trigger argument type | config argument type
