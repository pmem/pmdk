...
+ **POBJ_XALLOC_NO_ABORT** - if the function ends incorrectly, return error
...
+ **POBJ_XFREE_NO_ABORT** - if the function ends incorrectly, return error
number instead of aborting the transaction.
...
**TX_STAGE_WORK** it returns a handle to the newly allocated object. Otherwise,
when the **POBJ_XALLOC_NO_ABORT** flag is enabled the **OID_NULL** is returned,
otherwise when the flag is disabled, the stage is changed to **TX_STAGE_ONABORT**,
...
object. Otherwise, when the **POBJ_XALLOC_NO_ABORT** flag is enabled
the **OID_NULL** is returned, otherwise when the flag is disabled, the stage
is changed to **TX_STAGE_ONABORT**, **OID_NULL** is returned, and *errno* is set appropriately.
...
newly allocated object. Otherwise, when the **POBJ_XALLOC_NO_ABORT** flag is enabled
the **OID_NULL** is returned, otherwise when the flag is disabled, the stage
is changed to **TX_STAGE_ONABORT**, **OID_NULL** is returned, and *errno* is set appropriately.
...
returns 0. Otherwise, when the **POBJ_XFREE_NO_ABORT** flag is enabled the
error number is returned, when the flag is disabled the stage is changed to
...
Otherwise, when *flag* **POBJ_XALLOC_NO_ABORT** is enabled **OID_NULL** is returned
and *errno* is set appropriately. When *flag* is disabled the transaction is aborted,
the stage is changed to *TX_STAGE_ONABORT* and *errno* is set appropriately.
...
On success, **pmemobj_tx_free**() returns 0. Otherwise, the transaction
is aborted, the stage is changed to *TX_STAGE_ONABORT* and *errno* is set appropriately.

 **pmemobj_tx_xfree**() returns 0. Otherwise, when *flag* **POBJ_XFREE_NO_ABORT**
is enabled error number is returned, when *flag* is disabled the transaction
is aborted, the stage is changed to *TX_STAGE_ONABORT* and *errno* is set appropriately.
