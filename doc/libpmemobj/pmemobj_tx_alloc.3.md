...
+ **POBJ_XALLOC_NO_ABORT** - if the function ends incorrectly,
...
+ **POBJ_XFREE_NO_ABORT** - if the function ends incorrectly,
do not abort the transaction.
...
**TX_STAGE_WORK** it returns a handle to the newly allocated object. Otherwise,
when the **POBJ_XALLOC_NO_ABORT** flag is enabled *errno* is set appropriately
and **OID_NULL** is returned, otherwise when the flag is disabled, *errno* is set
appropriately and transaction is aborted.
...
Otherwise, the stage is changed to **TX_STAGE_ONABORT**, *errno* is set appropriately
and transaction is aborted, when **POBJ_XLOCK_NO_ABORT** flag is set **OID_NULL**
is returned.
 ...
 **pmemobj_tx_xfree**() returns 0. Otherwise, the stage is changed to **TX_STAGE_ONABORT**,
**errno** is set appropriately and transaction is aborted, when **POBJ_XFREE_NO_ABORT**
flag is set error number is returned.
