...
objects provided through the array of pointers to PMEMoids *oidv* with size *oidcnt*.
If an object from the provided array is selected to be moved to a
new location in the heap, it is reallocated and all provided pointers to
...
old or new location.
All objects and pointers to objects should belong to the pool *pop*, or, in
case of pointers, can also reside on volatile memory. Defragmentation across
...
can be non-zero, even if the **pmemobj_defrag**() return value indicated failure.
This is because the failure might have occured after some objects were already
processed.
