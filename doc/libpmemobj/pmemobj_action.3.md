...
bitmask of the following value:

+ **POBJ_XPUBLISH_NO_ABORT** - if the function ends incorrectly, return error
...
On success, **pmemobj_tx_xpublish**() returns 0, otherwise, when the
**POBJ_XPUBLISH_NO_ABORT** flag is enabled the error number is returned,
when the flag is disabled, the stage is changed to *TX_STAGE_ONABORT*,
