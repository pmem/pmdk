...
_WINUX(Windows,* removing one or more replicas.)
_WINUX(,=q=* removing one or more replicas,
...
_WINUX(Windows,=q=When adding or deleting replicas, the two pool set files can differ only in the
