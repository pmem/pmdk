...
+ **PMEMPOOL_FEAT_SHUTDOWN_STATE** - check if pool consistency was not harmed
by power failure. It requires **PMEMPOOL_FEAT_CKSUM_2K** to be enabled since
it stores additional metadata in the second 2KiB of pool part internal metadata.
...
1 if it is disabled. On error, it returns -1 and sets *errno* accordingly.
