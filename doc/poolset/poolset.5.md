...
If the option *SINGLEHDR* is used, only the first part in each replica contains
...
If the option *NOHDRS* is used, none of the pool parts contains internal
metadata. The effective size of such a replica is the sum of sizes of all its
part files.
...
The *SINGLEHDR* and *NOHDRS* options concern only local pool set files.
That is if one wants to create a pool set with one of the options and with remote
replicas, one has to add this option to the local pool set file as well as to
every single remote pool set file.
