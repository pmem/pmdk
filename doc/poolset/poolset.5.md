...
The directory must NOT contain files with extension *.pobj*, otherwise the
behavior is undefined. If a file created by the library within the directory is
in any way altered (resized, renamed) the behavior is undefined.
...
Only poolsets with *OPTION_NO_HDRS* can safely use directories.
