...
The **pmempool** invoked with *info* command analyzes an existing pool created by **NVML** libraries provided by **file** parameter. **file** can be either existing pool file or poolset file.
...
Poolset file can contain multiple replicas also remote ones but **pmempool** currently does not read any data from the remote replica. It prints only a remote node address and a remote replica descriptor.
