...
**pmem2_vm_reservation_extend**(), **pmem2_vm_reservation_extend**() - extends and shrinks
...
The **pmem2_vm_reservation_shrink**() function shrinks the reservation by an interval specified
with *offset* and *size* variables that correspond respectively to offset into the reservaion
and the size that the reservation has to be shrunk by. For the function to succeed the *size*
and *offset* has to be aligned to an appropriate allocation granularity, the interval designated
by these variables has to cover exact empty interval that belongs to the reservation, additionally
the interval has to always start from the beggining of the reservation or end at the reservation
end address.
