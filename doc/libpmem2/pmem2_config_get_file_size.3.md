...
**pmem2_config_get_file_size**() - query file size
...
pointed by file descriptor or handle stored in the *config* and puts it in
*\*size*.
...
On failure it doesn't modify *\*size* and returns one of the following errors:
...
* **-ENOMEM** - while trying to validate the file descriptor, **fstat**(2)
failed.

* **-EACCES**, **-EIO**, **-ELOOP**, **-ENAMETOOLONG**, **-ENOENT**,
**-ENOMEM**, **-ENOTDIR** - while trying to determine whether fd points to
a Device DAX  **realpath**(3) failed.

* **-EACCES**, **-EISDIR**, **-ELOOP**, **-EMFILE**, **-ENAMETOOLONG**,
**-ENFILE**, **-ENOENT**, **-ENOMEM**, **-ENOTDIR**, **-ENXIO** - while trying
to determine Device DAX's size, **open**(2) failed.

* **-ERANGE** - while trying to determine Device DAX's size, **strtoull**(3)
failed.
...
* **-ENOMEM**, **-EIO** - while trying to validate the file descriptor,
**fstat**(2) failed.
