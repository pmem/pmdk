...
of the calling process. This function requires providing arguments:

* The source of the mapping may be either an opened file descriptor or an opened
handle to a file.
...
**\_get_osfhandle**() manual pages.
    ...
    * Similarly, the handle has to be created using *GENERIC_READ* or
*(GENERIC_READ | GENERIC_WRITE)* desired access and passed via
**pmem2_config_set_handle**(3). For details please see the **CreateFileA**() and
**pmem2_config_set_handle**(3) manual pages. (Windows only)

    Only the last call from **pmem2_config_set_fd**(3) and
**pmem2_config_set_handle**(3) affects **pmem2_map**().

If the **pmem2_map**() function succeeds, creating a new mapping, it
