...
have to be checked.
- does the file still use the same *NVDIMMs set*? It is possible
...
In case it was detected file's *NVDIMMs set* has been altered, you have to
...
if ((old_usc == new_usc) && (old_device_id == new_device_id)) {
	...
	if (!file_in_use) {
		/* ... and the file has been closed correctly. */
		return;
	}

	/* ... but the file has not been closed correctly. */
	unsafe_shutdown_info_initialize(); /* reinitialize */
	return;
} else {

	/* the unsafe shutdown HAS occurred (or the file has been copied)... */

	if (!file_in_use) {
		/* ... but the file was not in use during that event. */
		unsafe_shutdown_info_initialize(); /* reinitialize */
		return;
	}
...
To implement such *unsafe shutdown* detection, you have to store in the file
three values:
...
It has to indicate *FALSE* if the file is not in use. Before performing
any other operation, you have to alter its value to *TRUE*. Similarly
when you have done everything and you will be closing the file you have to set
