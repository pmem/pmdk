...
change. It requires comparing the **USC** value read in the past (```old_usc```)
with the current **USC** value (```new_usc```). If ```new_usc > old_usc``` you
can assume *the unsafe shutdown* occurred.
...
false-positives. Since detecting *the unsafe shutdown* may induce rebuilding data
which is very expensive it may be worth to consider using an approach more
...
obtained using **pmem2_source_device_id**(3). It also has to be stored by the
...
/* old_usc is on the persistent medium so unsafe shutdown can't affect it. */
```
...
the algorithm will make a bad decision based on the inconsistent (invalid) values.
