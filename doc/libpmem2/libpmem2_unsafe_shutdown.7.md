---
layout: manual
Content-Style: 'text/css'
title: _MP(LIBPMEM2_UNSAFE_SHUTDOWN, 7)
collection: libpmem2
header: PMDK
date: pmem2 API version 1.0
...

[comment]: <> (SPDX-License-Identifier: BSD-3-Clause
[comment]: <> (Copyright 2019-2020, Intel Corporation)

[comment]: <> (libpmem2_unsafe_shutdown.7 -- man page for libpmem2 unsafe shutdown)

[NAME](#name)<br />
[SYNOPSIS](#synopsis)<br />
[DESCRIPTION](#description)<br />
[GRANULARITY](#unsafe-shutdown-detection)<br />
[CAVEATS](#deep-sync-required)<br />
[CAVEATS](#checksum)<br />
[SEE ALSO](#see-also)

# NAME #

**libpmem2_unsafe_shutdown** - libpmem2 unsafe shutdown

# SYNOPSIS #

```c
#include <libpmem2.h>
```

# DESCRIPTION #

In systems with the persistent memory support, *a power-fail protected domain*
covers a set of resources from which data will be flushed to *a persistent medium*
in case of *a power-failure*. Data stored on *the persistent medium* is preserved
across power cycles.

The feature to flush all data stored in *the power-fail protected domain* to
*the persistent medium* is hardware-guaranteed. Since this feature is
mission-critical, the persistent memory support also requires the possibility
to detect cases when hardware fails to fulfill its guarantees in the face of
a power failure. Such an event is called *an unsafe (dirty) shutdown*. In case of
*the unsafe shutdown*, data that has not been flushed to *the persistent medium*,
is lost.

To mitigate this risk NVDIMMs expose *an unsafe shutdown counter* (**USC**).
**USC** value is stored on each NVDIMM and its value is bumped up each time
the unsafe shutdown happens.

> **Note**: The *unsafe shutdown* may affect data stored on a device, in a file,
> in a set of files and in a mapping spanning only a part of a file.
> For simplicity sake, all of these cases in this document will be called *the file*.

# UNSAFE SHUTDOWN DETECTION #

A basic way of detecting *the unsafe shutdown* is by detecting the **USC** value
change. A current **USC** value can be obtained using **pmem2_source_device_usc**(3).
It must be stored for a reference (```old_usc```). Next time when *the file* will
be opened if a current **USC** value (```new_usc```) is bigger then the old one
you can assume *the unsafe shutdown* occurred.

The **USC**-only approach is valid and straightforward but may produce many
false-positives. Since handling *the unsafe shutdown* is very expensive it may
be worth to consider using an approach more immune to false-positives.

It is worth verifying whether *the file* is still on the same *NVDIMMs set*
(if it is possible to copy *the file*). If *the file* have been copied to a
different *NVDIMMs set* all the knowledge which have been collected have to be
invalidated e.g. ```old_usc```. A unique identifier of the *NVDIMMs set* may be
obtained using **pmem2_source_device_id**(3). It also has to be stored for a
reference (```old_device_id```).

A pseudo-code taking into account ```old_usc``` and ```old_device_id``` may look
as follows:

```c
if (!unsafe_shutdown_info_initialized) {
	unsafe_shutdown_info_initialize();
} else if (old_device_id == new_device_id) {
	if (old_usc == new_usc) {
		/* the unsafe shutdown has NOT occurred. */
	} else {
		/* the unsafe shutdown HAS occurred. The file might affected. */
	}
} else {
	/* the file has been moved, the USC value doesn't matter. */
	unsafe_shutdown_info_initialize(); /* reinitialize */
}
```

The last factor worth considering is whether *the file* was closed unexpectedly?
If *the file* has been closed cleanly, it's certain the data hasn't been affected.
A detection, whether *the file* was in use during *the unsafe shutdown*, may be
implemented using an additional variable ```file_in_use``` which is an indicator
whether the file is currently in use. Its value should be *FALSE* if the file is
not in use. Before performing any data modification, its value has to be altered
to *TRUE*. Similarly, when all modifications are done and *the file* will be
closed the ```file_in_use``` value has to be set to *FALSE*.

> **Note**: The *unsafe shutdown* detection should be the first thing done after
> reopening *the file*. This means it has to be done before setting
> ```pool_in_use``` variable to *TRUE*.

Taking into account all of the above factors, you can use the following
pseudo-code to implement a robust (and fine-grained) *unsafe shutdown* detection:

```c
if (!unsafe_shutdown_info_initialized) {
	unsafe_shutdown_info_initialize();
} else if (old_device_id == new_device_id) {
	if (old_usc == new_usc) {
		/* the unsafe shutdown has NOT occurred. */
	} else {

		/* the unsafe shutdown HAS occurred... */

		if (file_in_use) {
			/* ... and the file was in use. The file might be corrupted. */
		} else {
			/* ... but the file was not in use. Data is safe. */
			unsafe_shutdown_info_initialize(); /* reinitialize */
		}
	}
} else {
	/* the file has been moved, the USC value doesn't matter. */
	unsafe_shutdown_info_initialize(); /* reinitialize */
}
```

# DEEP SYNC REQUIRED #

A *deep sync* is a way of making sure all the data already persisted will be
preserved in the face of *the unsafe shutdown*. Because of this making use of
**pmem2_deep_sync**() function is crucial in making your data immune to
*the unsafe shutdown* and in implementing any algorithm detecting
*the unsafe shutdown*. For details how to use **pmem2_deep_sync**() please see
the **pmem2_deep_sync**(3) manual page.

An application which implements the basic way of detecting *the unsafe shutdown*
has to store the ```old_usc``` value on *the persistent medium*
(*the power-fail protected domain* is not enough since it cannot rely on it
implementing an algorithm of detecting its failure). To achieve this the
```old_usc``` value has to be *deep synced* e.g.:

```c
struct shutdown_info_basic {
	uint64_t old_usc;
} *info;

struct pmem2_source *src;
struct pmem2_map *map;
pmem2_persist_fn persist;

/* store a USC value */
pmem2_source_device_usc(src, &info->old_usc);

/* persist and deep sync the new value */
persist(info, sizeof(*info));
pmem2_deep_sync(map, info, sizeof(*info));

/* old_usc is on the persistent medium so no unsafe shutdown can affect it. */
```

Similarily ```old_device_id``` has to be *deep synced*. Things are a little bit
more complex regarding the ```file_in_use``` variable. It has to be *deep synced*
after setting its value to *TRUE* and before proceeding with the application but
also *the file* has to be *deep synced* before changing ```file_in_use``` value
to *FALSE*. A pseudo-code showing how to deal with it may look as follow:

```c
uint64_t *file_in_use;
void *first_page;

/* after opening the file*/
assert(*file_in_use == FALSE);
*file_in_use = TRUE;
persist(file_in_use, sizeof(*file_in_use));
pmem2_deep_sync(map, file_in_use, sizeof(*file_in_use));

/* the file is opened. It is safe to modify. */

/* deep sync all changes before closing the file */
pmem2_deep_sync(map, first_page, PAGE_SIZE);

/* closing the file */
*file_in_use = FALSE;
persist(file_in_use, sizeof(*file_in_use));
pmem2_deep_sync(map, file_in_use, sizeof(*file_in_use));

/* the file is closed cleanly. */
```

# CHECKSUM #

The last thing worth considering while implementing a robust *unsafe shutdown*
detection algorithm is a checksum. Aforementioned logic requires storing multiple
variables. To make reasonable use of them they must be consistent with
each other e.g. if *the unsafe shutdown* happens between storing ```old_usc```
and ```old_device_id``` their values are inconsistent so after reboot the,
algorithm will take bad decision base on the inconsistent (invalid) values.

To solve this issue it is a good idea to store a checksum along with the all
required variables. A valid way of doing this may look as follow (pseudo-code):

```c
struct shutdown_info {
	uint64_t old_usc;
	uint64_t old_device_id;
	uint64_t file_in_use;
	uint64_t checksum;
} *info;

/* invalidate old values */
info->checksum = 0;
persist(info, sizeof(*info));
pmem2_deep_sync(map, info, sizeof(*info));

/* store new values */
/* ... */

/* persist and deep sync the new values */
persist(info, sizeof(*info));
pmem2_deep_sync(map, info, sizeof(*info));

/* validate the new values */
info->checksum = custom_checksum((void *)info, INFO_SIZE_WITHOUT_CHKSUM);
persist(info, sizeof(*info));
pmem2_deep_sync(map, info, sizeof(*info));
```

# SEE ALSO #

**pmem2_deep_sync**(3), **pmem2_persist_fn**(3), **pmem2_source_device_id**(3),
**pmem2_source_device_usc**(3) and **<https://pmem.io>**
