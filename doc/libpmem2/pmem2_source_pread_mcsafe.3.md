...
Above functions are capable of detecting and handling bad block errors. When a
bad block is encountered, **pmem2_source_pread_mcsafe**() and **pmem2_source_pwrite_mcsafe**()
functions return corresponding errorcode.
...
provided source, only source created with **pmem2_source_from_fd**(3) is supported.

* **PMEM2_E_NOSUPP** - read/write operation isn't supported on running platform.
Those operations are supported only on posix systems.

* **PMEM2_E_NOSUPP** - read/write operation doesn't support devdax source yet.
>Note: Devdax support will be added in the furture.

Those operations can also return all errors from the underlying **pread**(2) and
**pwrite**(2) functions.
