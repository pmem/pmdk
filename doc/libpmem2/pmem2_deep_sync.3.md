...
from the *map* to be stored durably in the most reliable persistence domain available to software. In particular, on supported platforms, this enables the code not to
rely on automatic WPQ (write pending queue) flush on power failure (ADR).
...
* **PMEM2_E_NOSUPP** - platform has no facilities for deep synchronization of
device dax.

* **PMEM2_E_ERRNO** - a low level error occurred, preventing the
data synchronization from succeeding. Check *errno* for the exact error
information.
