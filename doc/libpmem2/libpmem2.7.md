...
use persist instructions specific for forced granularity and skip
...
at library initialization time, but on the first call to **pmem2_map**(3).
This means that **PMEM2_FORCE_GRANULARITY** may still be set or modified
by the program until the first attempt to map file.
