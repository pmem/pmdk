...
In the case of block storage devices (SSD, HDD), data can be written
using system API calls such as `msync()`, `fsync()` on Linux
or `FlushFileBuffers()` on Windows. When the function is invoked, the data
is flushed to the medium with *PAGE_SIZE* accuracy. In the **libpmem2** library,
this type of precision is called **PMEM2_GRANULARITY_PAGE**.

The second type of granularity is **PMEM2_GRANULARITY_CACHE_LINE**.
For systems with persistent memory support, data must be manually flushed
from CPU caches to the persistence domain. This action will prevent data
loss in case of a power failure.

Platforms supporting persistence memory feature provides persistence of
data on the memory controller, however, in the case of
...
instructions for flushing *cache lines* (e.g. *CLWB*, *CLFLUSHOPT*,
*CLFLUSH* for Intel x86_64 architecture). Besides, each of these instructions
must be followed by a serializing operation (e.g. *SFENCE*) to ensure the latest
cache flushing was completed and the order of stores was preserved.
...
*protected domain* described above, to CPU caches. This means cache flush
instructions are no longer needed and the data persistence is guaranteed by
the battery installed on the platform. Only serializing instruction is still
needed to keep write sequence correct.

The **libpmem2** exposes a *pmem2_granularity* enum which can be used for
specifying granularity requested by a user or reading effective granularity.
