...
The **libpmem2** library provides a number of functions for the flexible
use of non-volatile memory, but the basis of using **libpmem2** is to
understand the three main structures i.e. *struct pmem2_src source*,
*struct pmem2_config config* and *struct pmem2_map map*:

* *source* - an object describing the data source for mapping.
The data source can be file descriptor, file handle, or anonymous mapping.
API dedicated to create a *source* is: **pmem2_source_from_fd**(3),
 **pmem2_source_from_handle**(3), **pmem2_source_from_anon**(3).

* *config* - define parameters for a new mapping that will be created
using *source* object mentioned above. The configuration structure must always be
initialized, but the only required argument to set in the *config* is a *granularity*.
...
* *map* - map instance created by **pmem2_map_new**(3) using *source* and
*config* as an input parameters. For reading information about created mapping
the **libpmem2** provides functions like **pmem2_map_get_address**(3),
**pmem2_map_get_size**(3), **pmem2_map_get_store_granularity**(3) - for getting
address, size and effective mapping granularity.

Data flushing is also realized by functions read from the *map* structure.
To get proper function use **pmem2_get_flush_fn**(3), **pmem2_get_persist_fn**(3)
or **pmem2_get_drain_fn**(3).

Another feature of **libpmem2** is a set of routines for optimally copying
to persistent memory, and it is also provided by *map* getters
