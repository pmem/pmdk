...
of **Pagesize**. For Windows, *\offset* must match the memory allocation granularity
of the system - the offset must be a multiple of the allocation granularity.

# RETURN VALUE #

The **pmem2_config_set_offset**() function returns 0 on success or one from the error
...
for Linux or is not a multiple of the allocation granularity for Windows.
