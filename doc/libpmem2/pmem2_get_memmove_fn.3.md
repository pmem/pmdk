...
based on data size. See **PMEM_MOVNT_THRESHOLD** description in **libpmem2**(7) for
details.
...
Functions above, for the same *map*, always return the same function.
This means that it's safe to cache their return value. However, these functions
are very cheap (because a return value is precomputed), so caching may not
be necessary.

If two (or more) mappings share the same *pmem2_memmove_fn*, *pmem2_memset_fn*,
*pmem2_memcpy_fn* and they are adjacent to each other, it is safe to call this
function for a range spanning those mappings.
