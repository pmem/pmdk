...
is mapped directly from DAX Device (/dev/daxX.Y) without an intervening
file system.
...
>WARNING:
On Linux, **pmem_msync**() and **msync**(2) should never be used on
memory ranges mapped from Device DAX.  In case of any changes
in the range where **pmem_is_pmem**() returns true **pmem_persist**()
should be used to force the changes to be stored durably in persistent
memory.
