...
For this simple example, we're just going to write a simple string to a
...
This illustrates a bunch of the basics functions in libpmem2:
First of them, `pmem2_config_new()`, create config struct that
will be used for mapping. In this example, we only set the granularity
in the config (line 49), other values remain default.
However, libpmem2 provides multiple functions to configure target mapping,
eg. `pmem2_config_set_length()` to set length which will be used for mapping,
or `pmem2_config_set_offset` which will be used to map the contents from the
specified location of the source.

Second highlighted line 44 above call `pmem2_source_from_fd()`
...
In practice, it means that the program will flush data using e.g. CLFLUSH or
will use eADR feature when available on the platform, but if is not available,
then `msync()` is also accepted and no error will occur.
...
The last step just create a mapping using described above
config and source. Underlying function of `pmem2_map()` call `mmap(2)`
...
The libpmem2 function `pmem2_get_persist_fn` automatically determine
if the memory in the given map is really persistent memory or if
it is just a memory-mapped file on a traditional file system. Using
this call in your program you don't have to decide what to do when
given a non-pmem file as a source.
...
Belowe example illustrates that the following code:
